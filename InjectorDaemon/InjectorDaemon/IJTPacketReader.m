...
- (id)initWithPacket: (packet_t)packet
    ...
    if(self) {
        ...
        if(type == IJTPacketReaderTypeWiFi) {
            ...
            _sourceMacAddress = [NSString stringWithUTF8String:ether_ntoa((struct ether_addr *)&ethernet->ether_shost)];
            _destinationMacAddress = [NSString stringWithUTF8String:ether_ntoa((struct ether_addr *)&ethernet->ether_dhost)];
...
- (NSString *)getHexFormat {
    ...
    for(u_int16_t i = 0 ; i < self.packet.header.caplen ; i++) {
        ...
        dump = [dump stringByAppendingString:[NSString stringWithFormat:@"%02x", content[i]]];
    }
...
- (NSString *)getAcsiiFormat {
    ...
    for(u_int16_t i = 0 ; i < self.packet.header.caplen ; i++) {
        ...
        if(isalnum(content[i]) || isalpha(content[i])) {
            dump = [dump stringByAppendingString:[NSString stringWithFormat:@"%c", content[i]]];
        }
        else {
            dump = [dump stringByAppendingString:@"."];
        }
