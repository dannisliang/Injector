...
- (int)deleteTCPOrUDPRuleAtInterface: (NSString *)interface
                                  op: (IJTFirewallOperator)op
                           direction: (IJTFirewallDirection)dir
                               proto: (IJTFirewallProtocol)proto
                              family: (sa_family_t)family
                             srcAddr: (NSString *)srcAddr
                             dstAddr: (NSString *)dstAddr
...
- (int)deleteRule: (struct pfioc_rule)pdr
    ...
    char *device = pdr.rule.ifname;
    sa_family_t af = pdr.rule.af;
    u_int8_t action = pdr.rule.action;
    u_int8_t dir = pdr.rule.direction;
    u_int8_t proto = pdr.rule.proto;
    struct in_addr src = pdr.rule.src.addr.v.a.addr.v4;
    struct in_addr dst = pdr.rule.dst.addr.v.a.addr.v4;
    struct in_addr srcmask = pdr.rule.src.addr.v.a.mask.v4;
    struct in_addr dstmask = pdr.rule.dst.addr.v.a.mask.v4;
    u_int16_t srcport1 = ntohs(pdr.rule.src.xport.range.port[0]);
    u_int16_t srcport2 = ntohs(pdr.rule.src.xport.range.port[1]);
    u_int16_t dstport1 = ntohs(pdr.rule.dst.xport.range.port[0]);
    u_int16_t dstport2 = ntohs(pdr.rule.dst.xport.range.port[1]);
    IJTFirewallTCPFlag tcpFlags = pdr.rule.flags;
    IJTFirewallTCPFlag tcpFlagsMask = pdr.rule.flagset;
    u_int8_t type = pdr.rule.type - 1;
    u_int8_t code = pdr.rule.code - 1;
    BOOL keepstatue = pdr.rule.keep_state;
    BOOL quick = pdr.rule.quick;
...
- (struct pfioc_rule)searchRuleAtInterface: (NSString *)interface
    ...
    while (--n >= 0) {
        ...
        if(pr.rule.action == op && pr.rule.direction == dir &&
           ...
           pr.rule.src.addr.v.a.addr.v4.s_addr == inet_addr([srcAddr UTF8String]) &&
           pr.rule.src.addr.v.a.mask.v4.s_addr == inet_addr([srcMask UTF8String]) &&
           pr.rule.dst.addr.v.a.addr.v4.s_addr == inet_addr([dstAddr UTF8String]) &&
           pr.rule.dst.addr.v.a.mask.v4.s_addr == inet_addr([dstMask UTF8String]) &&
           ((pr.rule.keep_state && keepState) || (!pr.rule.keep_state && !keepState))) {
...
- (int)addRuleAtInterface: (NSString *)interface
    ...
    par.rule.src.addr.v.a.addr.v4.s_addr = inet_addr([srcAddr UTF8String]);
    par.rule.src.addr.v.a.mask.v4.s_addr = inet_addr([srcMask UTF8String]);
    par.rule.dst.addr.v.a.addr.v4.s_addr = inet_addr([dstAddr UTF8String]);
    par.rule.dst.addr.v.a.mask.v4.s_addr = inet_addr([dstMask UTF8String]);
...
- (int)tailRuleByExpression: (NSString *)rule {
    ...
    FILE *fp = fopen("/var/root/Injector/pf.conf", "a+");
    if(!fp)
        goto BAD;
    fprintf(fp, "%s\n", [rule UTF8String]);
    fclose(fp);
