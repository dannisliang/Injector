...
@property (nonatomic, strong) UITextView *acsiiDumpTextView;
@property (nonatomic, strong) NSAttributedString *hexDumpAttributedString;
@property (nonatomic, strong) NSAttributedString *acsiiDumpAttributedString;
...
- (void)viewDidLoad {
    ...
    //acsii dump texr view
    self.acsiiDumpTextView = [[UITextView alloc] init];
    self.acsiiDumpTextView.text = [self.reader getAcsiiFormat];
    self.acsiiDumpTextView.textColor = IJTValueColor;
    self.acsiiDumpTextView.font = [UIFont fontWithName:@"Menlo-Regular" size:14];
    //self.acsiiDumpTextView.selectable = NO;
    self.acsiiDumpTextView.editable = NO;
    self.acsiiDumpTextView.scrollEnabled = NO;
    height = [IJTFormatUITextView textViewHeightForAttributedText:self.acsiiDumpTextView.attributedText
                                                         andWidth:SCREEN_WIDTH - 16];
    [self.acsiiDumpTextView sizeToFit];
    xpotion = (SCREEN_WIDTH - CGRectGetWidth(self.acsiiDumpTextView.frame))/2;
    self.acsiiDumpTextView.frame = CGRectMake(xpotion, 0, CGRectGetWidth(self.acsiiDumpTextView.frame), height);
    self.acsiiDumpAttributedString =
    [[NSAttributedString alloc] initWithAttributedString:self.acsiiDumpTextView.attributedText];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.section == 0 && indexPath.row == 0) {
        ...
        [cell.contentView.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
        [cell.contentView addSubview:self.acsiiDumpTextView];
    ...
    switch (protocol) {
        ...
        case IJTPacketReaderProtocolARPRequest: {
            ...
            if(indexPath.row == 0) {
                ...
                fieldValue = [NSString stringWithFormat:@"%s", ether_ntoa(&arp->ar_sha)];
            }
            else if(indexPath.row == 6) {
                ...
                fieldValue = [NSString stringWithFormat:@"%s", ether_ntoa(&arp->ar_tha)];
            }
            else if(indexPath.row == 8) {
                dataLength = arp->ar_pln;
        ...
        case IJTPacketReaderProtocolTCPOverICMP: {
            ...
            if(indexPath.row == 0) {
                ...
                fieldValue = [NSString stringWithFormat:@"%d", ntohs(tcp->th_sport)];
            }
            else if(indexPath.row == 1) {
                dataLength = 2;
                fieldName = @"Destination Port";
                fieldValue = [NSString stringWithFormat:@"%d", ntohs(tcp->th_dport)];
            }
            else if(indexPath.row == 2) {
                dataLength = 4;
                fieldName = @"Sequence Number";
                fieldValue = [NSString stringWithFormat:@"%u", ntohl(tcp->th_seq)];
            }
            else if(indexPath.row == 3) {
                dataLength = 4;
                fieldName = @"Ack Number";
                fieldValue = [NSString stringWithFormat:@"%u", ntohl(tcp->th_ack)];
            }
            else if(indexPath.row == 4) {
                dataLength = 1;
                fieldName = @"Header Length";
                ...
                fieldValue = [NSString stringWithFormat:@"%u", ntohl(tcp->th_win)];
            }
            else if(indexPath.row == 8) {
                dataLength = 2;
                fieldName = @"Checksum";
        ...
        case IJTPacketReaderProtocolUDPOverICMP: {
            ...
            if(indexPath.row == 0) {
                fieldName = @"Source Port";
                fieldValue = [NSString stringWithFormat:@"%d", ntohs(udp->uh_sport)];
                dataLength = 2;
            }
            else if(indexPath.row == 1) {
                fieldName = @"Destination Port";
                fieldValue = [NSString stringWithFormat:@"%d", ntohs(udp->uh_dport)];
                dataLength = 2;
            }
            else if(indexPath.row == 2) {
                fieldName = @"Length";
        ...
        case IJTPacketReaderProtocolWOL: {
            ...
            if(indexPath.row == 0) {
                ...
                fieldValue = [NSString stringWithUTF8String:ether_ntoa((struct ether_addr *)&wol->wol_ether_addr)];
            }
            else if(indexPath.row == 2) {
                dataLength = 6;
                fieldName = @"Password";
                fieldValue = [NSString stringWithUTF8String:ether_ntoa((struct ether_addr *)&wol->wol_password)];
            }
        }
            break;
        case IJTPacketReaderProtocolIPv6: {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.acsiiDumpTextView.attributedText = self.acsiiDumpAttributedString;
...
selectDataAtIndexPath: (NSIndexPath *)indexPath
    ...
    [IJTFormatUITextView textView:self.acsiiDumpTextView
                      selectRange:NSMakeRange(selectLocation, dataLength)
                  selectTextColor:IJTWhiteColor
            selectBackgroundColor:[IJTColor lighter:IJTToolsColor times:2]
                     inverseRange:NSMakeRange(inverseLocation, inverseLength)
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.section == 0) {
        ...
        return CGRectGetHeight(self.acsiiDumpTextView.frame);
    }
    return [super tableView:tableView heightForRowAtIndexPath:indexPath];
}
