...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
    if(section == 0) {
        if(self.searchController.isActive)
            return [NSString stringWithFormat:@"Search Detect Event(%lu)", (unsigned long)self.searchResultDetectEventArray.count];
        else
            return [NSString stringWithFormat:@"Detect Event(%lu)", (unsigned long)self.detectEventArray.count];
    }
    return @"";
}
