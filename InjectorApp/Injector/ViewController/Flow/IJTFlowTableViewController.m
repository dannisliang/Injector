...
- (void) loadData
{
    ...
    for(int i = 0 ; i < 2 ;i++) {
        NSString *jsonstring = [IJTHTTP retrieveFrom:@"RetrieveTimeRange.php"
...
DETECT:
    for(int i = 0 ; i < 2 ; i++) {
        //detect event
        jsonstring = [IJTHTTP retrieveFrom:@"RetrieveDetectEvent.php" post:[NSString stringWithFormat:@"SerialNumber=%@&StartTime=%ld&EndTime=%ld", self.serialNumber, start, end] timeout:7];
...
- (void)hsDatePickerPickedDate:(NSDate *)date {
    ...
    [IJTDispatch dispatch_main_after:DISPATCH_DELAY_TIME block:^{
        NSString *updateString = [NSString stringWithFormat:@"Retrieving all the time flow data\n" \
                                  "From: %@\n" \
                                  "To: %@\n"
                                  "and Drawing...",
                                  [IJTFormatString formatTime:_selectedStartTime],
...
- (IBAction)setAllTheDate:(id)sender {
    [KVNProgress showWithStatus:@"Retrieving all the time flow data and Drawing..."];
    [self.tableView setUserInteractionEnabled:NO];
    [IJTDispatch dispatch_main_after:DISPATCH_DELAY_TIME block:^{
