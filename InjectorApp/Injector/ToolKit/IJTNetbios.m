...
- (int)readTimeout: (u_int32_t)timeout
    ...
    while(1) {
        ...
        unitID = [NSString stringWithUTF8String:
                  (const char *)ether_ntoa((const struct ether_addr *)netbios_data)];
        
        if(netbioscallback) {
            netbioscallback(target, selector, names, unitID, source, object);
        }
        else {
