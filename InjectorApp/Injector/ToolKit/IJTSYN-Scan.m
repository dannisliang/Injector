...
@property (nonatomic, strong) NSMutableArray *recvAckArray;
@property (nonatomic) int bpfbufsize;
@property (nonatomic) int data_link_len;

@end
@implementation IJTSYN_Scan
...
- (void)open: (NSString *)interface {
    ...
    if([interface isEqualToString:@"en0"]) {
        ...
        /* dump tcp RST only */
        // tcpdump -i en0 -dd "tcp[13:1] == 0x12 or tcp[tcpflags] & tcp-rst != 0"
        ...
        /* dump tcp RST only */
        // tcpdump -i pdp_ip0 -dd "tcp[13:1] == 0x12 or tcp[tcpflags] & tcp-rst != 0"
        struct bpf_insn insns[] = {
            { 0x20, 0, 0, 0x00000000 },
            { 0x15, 0, 9, 0x02000000 },
            { 0x30, 0, 0, 0x0000000d },
...
- (int)injectWithInterval: (useconds_t)interval
    ...
    self.recvAckArray = [[NSMutableArray alloc] init];
    ssize_t size;
    struct packet_tcp send_packet;
    int s = -1;
    SYNScanCallback synscancallback = NULL;
    ...
    if(target && selector) {
        synscancallback = (SYNScanCallback)[target methodForSelector:selector];
    }
    ...
    for(_current_index = 0 ; _current_index < total ; _current_index++) {
        ...
        int iplen = tcp_headear_option + (sizeof send_packet.ip) + (sizeof send_packet.tcp);
        
        //send syn
        memset(&send_packet, 0, sizeof(send_packet));
        memcpy(send_packet.data, "\x02\x04\x05\xb4", 4);
    ...
    self.recvAckArray = [NSMutableArray arrayWithArray:[self.recvAckArray sortedArrayUsingSelector:@selector(compare:)]];
    
    for(u_int16_t port = _start_port ; port <= _end_port ; port++) {
        IJTSYN_ScanFlags flags = IJTSYN_ScanFlagsFiltered;
        for(NSNumber *openPort in self.recvAckArray) {
            if([openPort unsignedShortValue] == port) {
                flags = IJTSYN_ScanFlagsOpen;
                break;
            }
        }
        for(NSNumber *closePort in self.recvRstArray) {
            if([closePort unsignedShortValue] == port) {
                flags = IJTSYN_ScanFlagsClose;
                break;
            }
        }
        if(synscancallback) {
            synscancallback(target, selector, port, [self portName:port], flags, object);
        }
        else {
    ...
    while(1) {
        ...
        while(bpf_len > 0) {
            ...
            if(tcp->th_flags == (TH_SYN | TH_ACK)) {
                [self.recvAckArray addObject:@(htons(tcp->th_sport))];
            }
...
static void make_tcp_header(struct packet_tcp *packet, int src_ip, int src_port,
                            int dst_ip, int dst_port, int seq, int ack, u_int8_t flags, int tcp_headear_option) {
    ...
    packet->tcp.th_off   = (sizeof(struct tcphdr) + tcp_headear_option) >> 2;
    ...
    packet->ip.ip_sum    = htons((sizeof packet->tcp) + tcp_headear_option);
