...
- (int)injectWithInterval: (useconds_t)interval
    ...
    int s = -1;
    MaimonScanCallback maimonscancallback = NULL;
    ...
    if(target && selector) {
        maimonscancallback = (MaimonScanCallback)[target methodForSelector:selector];
    }
    ...
    self.recvIcmpArray = [NSMutableArray arrayWithArray:[self.recvIcmpArray sortedArrayUsingSelector:@selector(compare:)]];
     ...
     for(u_int16_t port = _start_port ; port <= _end_port ; port++) {
         IJTMaimon_ScanFlags flags = IJTMaimon_ScanFlagsFiltered | IJTMaimon_ScanFlagsOpen;
         for(NSNumber *openPort in self.recvIcmpArray) {
             if([openPort unsignedShortValue] == port) {
                 flags = IJTMaimon_ScanFlagsFiltered;
                 break;
             }
         }
         for(NSNumber *closePort in self.recvRstArray) {
             if([closePort unsignedShortValue] == port) {
                 flags = IJTMaimon_ScanFlagsClose;
                 break;
             }
         }
         if(maimonscancallback) {
             maimonscancallback(target, selector, port, [self portName:port], flags, object);
         }
         else {
