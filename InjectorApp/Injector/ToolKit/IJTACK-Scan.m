...
- (void)open: (NSString *)interface {
    ...
    if([interface isEqualToString:@"en0"]) {
        ...
        /* dump tcp RST only */
        // tcpdump -i en0 -dd "tcp[tcpflags] & tcp-rst != 0"
        ...
        /* dump tcp RST only */
        // tcpdump -i pdp_ip0 -dd "tcp[tcpflags] & tcp-rst != 0"
        struct bpf_insn insns[] = {
            { 0x20, 0, 0, 0x00000000 },
            { 0x15, 0, 8, 0x02000000 },
            { 0x30, 0, 0, 0x0000000d },
...
- (int)injectWithInterval: (useconds_t)interval
    ...
    ACKScanCallback ackscancallback = NULL;
    int s = -1;
    ...
    if(target && selector) {
        ackscancallback = (ACKScanCallback)[target methodForSelector:selector];
    }
    ...
    for(u_int16_t port = _start_port ; port <= _end_port ; port++) {
        BOOL unfiltered = NO;
        for(NSNumber *unfilteredPort in self.recvRstArray) {
            if([unfilteredPort unsignedShortValue] == port) {
                unfiltered = YES;
                break;
            }
        }
        ...
        if(unfiltered) {
            if(ackscancallback) {
                ackscancallback(target, selector, port, [self portName:port], IJTACK_ScanFlagsUnfiltered, object);
            }
            else
                printf("unfitlered: %d\n", port);
        }
            ...
            if(ackscancallback) {
                ackscancallback(target, selector, port, [self portName:port], IJTACK_ScanFlagsFiltered, object);
            }
            else
                printf("filter: %d\n", port);
        }
